...
#    status = st.radio("Please select", ("Inventory Projection"))
    ...
    for pid in pid_80_oc:
        st.table(projection_final[projection_final.product_id == pid].style.apply(highlight_greaterthan, threshold=10, column='days_on_hand', axis=1).format("{:.0%}",subset=num_cols))
